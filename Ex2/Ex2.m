...
dim   = 50; % Number of optimization variables
...
while kappa > opt.epsilon % Stop once the barrier term is small
  ...
  while 1
    ...
    hesPhi = prob.G'*diag(1./(prob.d-prob.G*z).^2)*prob.G;
    Dz = (prob.H + kappa*hesPhi)\(-prob.H*z - prob.q -  kappa*prob.gradPhi(z));
  ...
  kappa = kappa * opt.mu(i);
end
newt_step(i) = stats.innerIterations;
